...
import { buildRecapText, recapRows } from '../../domain/practice';
...
function kindLabel(kind: 'album' | 'folder' | 'track'): string {
  if (kind === 'album') {
    return 'Album';
  }
  if (kind === 'folder') {
...
export function LessonRecapScreen() {
  ...
  const tracks = useMemo(() => {
    ...
    return useLibraryStore.getState().tracksIn(kind, id);
  ...
  const title =
    kind === 'album'
      ...
      : kind === 'folder'
        ...
        : allTracks.find((item) => item.id === id)?.title;
  const name = title?.trim() || (kind ? kindLabel(kind) : '');
  const rows = useMemo(() => recapRows(tracks, markersByTrackId), [tracks, markersByTrackId]);
  const shareText = useMemo(() => buildRecapText(name, rows), [name, rows]);
  const showTrackTitle = tracks.length > 1;
        ...
        {rows.length === 0 ? (
          <View style={styles.emptyBox}>
            <Text style={styles.empty}>
              Qui compariranno i momenti toccati, con la prima riga di ogni appunto.
            </Text>
          </View>
        ) : (
          <>
