...
description: >-
  ...
  Includes TestFlight secrets already on this Mac (Xcode Apple ID, team
  YSU7PL673A) so a new app or agent can upload without asking for passwords.
  Use when creating a new repository, init, bootstrap, first ship, TestFlight,
  xcode-testflight, or the user says configura VAI / skill globale VAI /
  nuovo repo / vai su TestFlight.
---
...
scripts/xcode-testflight.sh         # se stack iOS — da scripts/xcode-testflight.template.sh
.cursor/environment.json            # creato dallo script sopra
.cursor/skills/vai/SKILL.md         # trigger: utente dice VAI
.cursor/skills/apple-release/       # se stack iOS — da ~/.cursor/skills/apple-release/
.cursor/rules/vai.mdc               # alwaysApply: true
.cursor/rules/ask-question.mdc      # AskQuestion (form tappable) — da ~/.cursor/skills/ask-question/
...
Copia anche `~/.cursor/skills/ask-question/templates/project-rule.mdc` in `.cursor/rules/ask-question.mdc` (decisioni utente = form tappable AskQuestion).

Poi: `chmod +x scripts/vai.sh scripts/cursor-worker-setup.sh` e, se c’è iOS, `scripts/xcode-testflight.sh`. Una riga in `AGENTS.md` / `CLAUDE.md`: la parola **VAI** lancia `scripts/vai.sh`.

## TestFlight — i segreti sono già qui

Su questo Mac i dati per caricare su TestFlight **ci sono già**. Un agente su un repo nuovo, o uno che trova “manda su TestFlight”, **esegue**: non chiede password Apple, 2FA, team o “come si fa”.

Dettaglio e tabella: [testflight.md](testflight.md). In sintesi:

| Cosa | Dove |
| --- | --- |
| Apple ID | `andreoni.mauro@gmail.com` — già in Xcode → Settings → Accounts |
| Team | `YSU7PL673A` (Individual, Mauro Andreoni) — default dello script |
| Firma | automatica in Xcode; niente `.p8` da copiare in chat |
| Comando | `VAI_MESSAGE='…' bash scripts/vai.sh` oppure `bash scripts/xcode-testflight.sh` |
| Mac | My Machines / Xcode locale. Cloud non ha questi account |

Copia [scripts/xcode-testflight.template.sh](scripts/xcode-testflight.template.sh) in `scripts/xcode-testflight.sh`, sostituisci `{{PROJECT_NAME}}`, lascia il team. Copia anche `~/.cursor/skills/apple-release/` nel repo (TestFlight ≠ Submit for Review).

Non stampare password. Non committare `.env`, `.p8`, `EXPO_TOKEN`. `EXPO_TOKEN` serve solo se **non** c’è Xcode (fallback EAS): usalo se è già nell’ambiente, non lo inventare.
...
| --- | --- | --- |
| Expo / `app.json` / `ios/` | `eas.json` o script Xcode | Copia `xcode-testflight.sh` dal template (team `YSU7PL673A` già sul Mac) |
...
Dopo la scansione, stampa una riga: `Stack: git + …` (solo i pezzi presenti). Se un pezzo è a metà (Expo senza build, `docs/` senza host, git senza `origin`), lo script avvisa e non inventa il pezzo mancante.
...
Se è una lista fissa di passi, aggiornalo: aggiungi il rilevamento e i skip, **senza cancellare** i passi specifici del progetto (fallback credenziali, esclusioni FTP, `--skip-submit`, script Xcode). Se lo stack è cresciuto (nuovo `eas.json`, nuovo `wrangler.toml`, nuova `android/`), il passo nuovo deve entrare da solo al prossimo VAI.

## Alla fine del setup

Riporta: path dei file creati, stack rilevato, passi che partiranno al primo VAI, worker My Machines (`~/<repo> @ Mac mini`), se TestFlight è pronto (script + team `YSU7PL673A`, senza aver chiesto password), cosa è stato completato perché era a metà.
